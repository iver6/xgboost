...
#include <thrust/execution_policy.h>  // thrust::musa::par
#include <thrust/functional.h>        // thrust::plus<>
#include <thrust/iterator/counting_iterator.h>
#include <thrust/transform_reduce.h>

#include "../common/device_helpers.muh"
...
PackedReduceResult Reduce(Context const* ctx, MetaInfo const& info, Fn&& loss) {
  ...
  if (ctx->IsMUSA()) {
    ...
    result = thrust::transform_reduce(
        thrust::musa::par(alloc), begin, end,
...
class PseudoErrorLoss : public MetricNoCache {
 ...
 public:
  ...
  double Eval(const HostDeviceVector<bst_float>& preds, const MetaInfo& info) override {
    ...
    auto predts = ctx_->IsMUSA() ? preds.ConstDeviceSpan() : preds.ConstHostSpan();
    info.weights_.SetDevice(ctx_->Device());
    common::OptionalWeights weights(ctx_->IsMUSA() ? info.weights_.ConstDeviceSpan()
