...
#include "../common/algorithm.muh"        // SegmentedArgSort
#include "../common/optional_weight.h"    // OptionalWeights
#include "../common/threading_utils.muh"  // UnravelTrapeziodIdx,SegmentedTrapezoidThreads
#include "auc.h"
...
std::tuple<double, double, double> GPUBinaryAUC(Context const *ctx,
  ...
  dh::safe_cuda(musaSetDevice(ctx->Ordinal()));
  ...
  dh::Iota(d_unique_idx, ctx->MUSACtx()->Stream());
  ...
  auto end_unique = thrust::unique_by_key_copy(
      thrust::musa::par(alloc), uni_key, uni_key + d_sorted_idx.size(),
  ...
  double auc = thrust::reduce(thrust::musa::par(alloc), in, in + d_unique_idx.size());
...
double ScaleClasses(Context const *ctx, bool is_column_split, common::Span<double> results,
  ...
  if (collective::IsDistributed() && !(is_column_split && collective::IsFederated())) {
    ...
    CHECK_EQ(dh::MUSAGetPointerDevice(results.data()), device);
  ...
  thrust::tie(auc_sum, tp_sum) =
      thrust::reduce(ctx->MUSACtx()->CTP(), reduce_in, reduce_in + n_classes, Pair{0.0, 0.0},
...
void SegmentedReduceAUC(common::Span<size_t const> d_unique_idx,
  ...
  thrust::reduce_by_key(thrust::musa::par(alloc), key_in,
...
std::pair<double, std::uint32_t> GPURankingAUC(Context const *ctx, common::Span<float const> predts,
  ...
  size_t n_valid = thrust::count_if(
      thrust::musa::par(alloc), check_it, check_it + group_ptr.size() - 1,
   ...
   */
  double auc = thrust::reduce(thrust::musa::par(alloc), dh::tbegin(s_d_auc),
                              dh::tend(s_d_auc), 0.0);
...
std::tuple<double, double, double> GPUBinaryPRAUC(Context const *ctx,
  ...
  thrust::tie(total_pos, total_neg) =
      thrust::reduce(thrust::musa::par(alloc), it, it + labels.Size(),
...
double GPUMultiClassPRAUC(Context const *ctx, common::Span<float const> predts,
  ...
  dh::XGBCachingDeviceAllocator<char> alloc;
  thrust::reduce_by_key(thrust::musa::par(alloc), key_it,
...
std::pair<double, uint32_t> GPURankingPRAUCImpl(Context const *ctx,
  ...
  uint32_t invalid_groups;
  {
    ...
    thrust::tie(auc, invalid_groups) = thrust::reduce(
        thrust::musa::par(alloc), it, it + n_groups,
...
std::pair<double, std::uint32_t> GPURankingPRAUC(Context const *ctx,
  ...
  dh::XGBDeviceAllocator<char> alloc;
  auto labels = info.labels.View(ctx->Device());
  if (thrust::any_of(thrust::musa::par(alloc), dh::tbegin(labels.Values()),
